    ...
    <div class="help-card">
      ...
      <div class="help-body">
        ...
        <div class="nav-sidebar">
          ...
          <div 
            class="nav-pill" 
            :class="{ active: activeTab === 'dubbing' }"
            @click="switchTab('dubbing')"
          >
...
import { 
  Compass, Microphone, CopyDocument, Monitor, EditPen, CaretRight 
...
.nav-pill:hover { background: #eef1f6; }
.nav-pill.active {
  background-color: #5865f2; /* 稍亮一点的紫色 */
  color: #fff;
...
.video-thumb {
  width: 100%; height: 100%; background: #d9d9d9;
  display: flex; align-items: center; justify-content: center;
}
.play-btn { font-size: 40px; color: #5865f2; background: #fff; border-radius: 50%; padding: 5px; }
.duration {
  position: absolute; bottom: 10px; right: 10px;
  background: rgba(0,0,0,0.6); color: #fff; padding: 2px 6px; border-radius: 4px; font-size: 12px;
}

/* 意见反馈表单 */
.feedback-box {
  background: #f8faff; /* 浅蓝背景 */
  padding: 30px;
  border-radius: 12px;
}
...
.f-textarea :deep(.el-textarea__inner) {
  border: none; border-radius: 8px; padding: 15px; font-size: 14px;
}
.f-footer { display: flex; justify-content: flex-end; }
.submit-btn {
  background-color: #5362bc; border-color: #5362bc; padding: 12px 40px;
}
</style>