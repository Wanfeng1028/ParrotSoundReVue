...
```bash
VITE_ENABLE_FRONTEND_DEMO=true
```
...
- 后端开发模式：`npm run dev:backend`
- 后端启动模式：`npm run start:backend`
...
仅当 `VITE_ENABLE_FRONTEND_DEMO=true` 时，可使用以下纯前端验收账号：
...
- 前端生产构建：`npm --prefix parrot-frontend run build`
